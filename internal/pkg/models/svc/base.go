...
func NewDBWrapper(db *gorm.DB,logger *zap.Logger) *DBExtension {
	return &DBExtension{
		DB: db,
		logger: logger.With(zap.String("type", "model.svc")),
	}
}
...
func (dw *DBExtension) SetLogger(logger *zap.Logger) {
	dw.logger = logger
}
...
func (dw *DBExtension) getListCore(result interface{}, order string, limit, offset int, query interface{}, args []interface{}) error {
	...
	if err := db.Find(result).Error; err != nil {
		dw.logger.Error("mysql", zap.String("err",err.Error()), zap.String("message",fmt.Sprintf("failed to query %s, query is %+v, args are %+v, order is %s, limit is %d", tableNameAble.TableName(), query, args, order, limit)))
		return err
	}
...
func (dw *DBExtension) SaveOne(value TableNameAble) error {
	...
	if err = dw.Save(value).Error; err != nil {
		dw.logger.Error("mysql", zap.String("error", err.Error()), zap.String("message",fmt.Sprintf("Failed to save %s, the value is %+v", tableNameAble.TableName(), value)))
	}
	return err
}
...
func (dw *DBExtension) Update(attrs interface{}, query interface{}, args ...interface{}) error {
	...
	if err = db.Error; err != nil {
		dw.logger.Error("mysql",  zap.String("err", err.Error()), zap.String("message",fmt.Sprintf("failed to update %s, query is %+v, args are %+v, attrs is %+v", tableName, query, args, attrs)))
	}

	if db.RowsAffected == 0 {
		dw.logger.Warn("mysql",zap.String("message",fmt.Sprintf("No rows is updated.For %s, query is %+v, args are %+v, attrs is %+v", tableName, query, args, attrs)) )
	}
...
func (dw *DBExtension) GetOne(result interface{}, query interface{}, args ...interface{}) (found bool, err error) {
	...
	if err == gorm.ErrRecordNotFound {
		dw.logger.Info("mysql",zap.String("message",fmt.Sprintf("record not found for query %s, the query is %+v, args are %+v", tableNameAble.TableName(), query, args)) )
		return false, nil
	}

	if err != nil {
		dw.logger.Error("mysql", zap.String("err",err.Error()), zap.String("message",fmt.Sprintf("failed to query %s, the query is %+v, args are %+v", tableNameAble.TableName(), query, args)))
		return false, err
	}
...
func (dw *DBExtension) ExecSql(result interface{}, sql string, args ...interface{}) error {
	...
	if err != nil {
		dw.logger.Error("mysql", zap.String("err",err.Error()), zap.String("message",fmt.Sprintf("failed to execute sql %s, args are %+v", sql, args)))
	}
...
func (dw *DBExtension) countCore(count *int, byField string, query interface{}) error {
	...
	if err := db.Count(count).Error; err != nil {
		dw.logger.Error("mysql", zap.String("err",err.Error()), zap.String("message",fmt.Sprintf("failed to count %s, query is %+v, byField is %s", tableNameAble.TableName(), query, byField)))
		return err
	}
